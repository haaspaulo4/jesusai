...
function buildSystemPrompt(persona, lang, contextStr, memoryStr, profileStr, userName, isGroup, knowledgeSources, businessStr) {
  ...
  const hasBiblicalContext = knowledgeSources === null || (knowledgeSources && knowledgeSources.some(s => s.includes('bible') || s.includes('biblia')));
  const langInstructions = {
    'pt-BR': hasBiblicalContext
      ? 'REGRA CRÍTICA: Você DEVE responder no IDIOMA que a pessoa está usando. Se ela escreve em português, responda em português. Se em inglês, responda em inglês. Se em espanhol, responda em espanhol. NUNCA escreva em caracteres chineses. Isso é uma regra absoluta.\n\nREGRA ABSOLUTA: O conhecimento e versículos fornecidos no CONTEXTO abaixo JÁ ESTÃO DISPONÍVEIS para você. NUNCA diga "deixe-me buscar", "vou procurar", "espere enquanto busco" ou qualquer variação. Responda DIRETAMENTE usando o contexto fornecido.'
      : 'REGRA CRÍTICA: Você DEVE responder no IDIOMA que a pessoa está usando. Se ela escreve em português, responda em português. Se em inglês, responda em inglês. Se em espanhol, responda em espanhol. NUNCA escreva em caracteres chineses. Isso é uma regra absoluta.',
    'en-US': hasBiblicalContext
      ? 'CRITICAL: You MUST respond in the SAME LANGUAGE the person is using. If they write in English, respond in English. If in Portuguese, respond in Portuguese. If in Spanish, respond in Spanish. NEVER output Chinese characters. This is an absolute rule.\n\nABSOLUTE RULE: The knowledge and verses provided in the CONTEXT below are ALREADY AVAILABLE to you. NEVER say "let me search", "I will look up", "wait while I find" or any variation. Respond DIRECTLY using the provided context.'
      : 'CRITICAL: You MUST respond in the SAME LANGUAGE the person is using. If they write in English, respond in English. If in Portuguese, respond in Portuguese. If in Spanish, respond in Spanish. NEVER output Chinese characters. This is an absolute rule.',
    'es-ES': hasBiblicalContext
      ? 'REGLA CRÍTICA: Debes responder en el MISMO IDIOMA que la persona está usando. Si escribe en español, responde en español. Si en portugués, en portugués. Si en inglés, en inglés. NUNCA uses caracteres chineses. Esto es una regla absoluta.\n\nREGLA ABSOLUTA: El conocimiento y versículos proporcionados en el CONTEXTO abajo YA ESTÁN DISPONIBLES para ti. NUNCA digas "déjame buscar", "voy a buscar" o cualquier variación. Responde DIRECTAMENTE usando el contexto proporcionado.'
      : 'REGLA CRÍTICA: Debes responder en el MISMO IDIOMA que la persona está usando. Si escribe en español, responde en español. Si en portugués, en portugués. Si en inglés, en inglés. NUNCA uses caracteres chineses. Esto es una regla absoluta.',
  };
